...
import { dummyMaintenance, MAINTENANCE_COLUMNS } from '../data/dummyData.js';
import { approveMaintenance, rejectMaintenance } from '../services/maintenanceService.js';
...
export default function Maintenance() {
  const [requests, setRequests] = useState(dummyMaintenance);
  ...
  async function handleReject(reason) {
    if (!reason) return; // rejection requires a reason
  ...
  return (
    <div className="space-y-8">
                ...
                {items.map((r) => (
                  <div key={r.id} className="rounded-card border border-line bg-surface p-4 shadow-card">
                    ...
                    <div className="mt-2 flex items-center justify-between text-xs">
                      <span className={`font-medium uppercase ${PRIORITY_CLASSES[r.priority]}`}>
                        {r.priority}
                      </span>
      ...
      <ApprovalModal
        ...
        subject={modalRequest ? `${modalRequest.asset_name} (${modalRequest.asset_tag}) — ${modalRequest.issue_description}` : ''}
